...
class Categories:
    ...
    class Model:
        ...
        def get_total(self, search=None):
            try:                
                ...
                if not search:
                    query = """SELECT count(*) AS total FROM systems_categories AS s;"""
                else:
                    query = """SELECT count(*) AS total FROM systems_categories AS s WHERE (s."name" ILIKE '%{s}%' OR s."name_en" ILIKE '%{s}%');""".format(s=search)
                cur.execute(query)
                row = cur.fetchone()
                cur.close()
                if row:                    
        ...
        def get_rows(self, c_page=1, p_size=10, search=None):
            try:
                cur = self.db.cursor(cursor_factory=RealDictCursor)
                if not search:
                    query = """SELECT * FROM systems_categories AS s ORDER BY "name" ASC LIMIT {l} OFFSET {o};""".format(l=p_size, o=(c_page - 1) * p_size)
                else:
                    query = """SELECT * FROM systems_categories AS s  WHERE (s."name" ILIKE '%{s}%' OR s."name_en" ILIKE '%{s}%') ORDER BY "name" ASC LIMIT {l} OFFSET {o};""".format(l=p_size, o=(c_page - 1) * p_size, s=search)
                cur.execute(query)
                rows = cur.fetchall()
                cur.close()
                if rows:
        ...
        def get_row(self, id):
            try:
                cur = self.db.cursor(cursor_factory=RealDictCursor)
                query = """SELECT * FROM systems_categories AS c WHERE c.id = %s;""" % id
                cur.execute(query)
                row = cur.fetchone()
                cur.close()
                if row:
        ...
        def get_categories(self, id):
            try:
                cur = self.db.cursor(cursor_factory=RealDictCursor)
                query = """SELECT * FROM systems_categories_categories AS c WHERE c.system_id = %s ORDER BY "order" ASC;""" % id
                cur.execute(query)
                rows = cur.fetchall()
                cur.close()
                if rows:
        ...
        def update_row(self, id, nm, nm_en):
            try:
                cur = self.db.cursor(cursor_factory=RealDictCursor)
                query = """UPDATE systems_categories SET "name" = '%s', "name_en" = '%s' WHERE id = %s;""" % (
                    nm, nm_en, id)
                cur.execute(query)
                self.db.commit()
                return True
        ...
        def update_row_visibility(self, id, status):
            try:
                cur = self.db.cursor(cursor_factory=RealDictCursor)
                query = """UPDATE systems_categories SET "visible" = '%s' WHERE id = %s;""" % (
                    status, id)
                cur.execute(query)
                self.db.commit()
                return True
        ...
        def update_row_metadata(self, id, desc):
            try:
                cur = self.db.cursor(cursor_factory=RealDictCursor)
                query = """UPDATE systems_categories SET "description" = '%s' WHERE id = %s;""" % (
                    desc, id)
                cur.execute(query)
                self.db.commit()
                return True
    ...
    class CRowSaveMetadata(Resource):
